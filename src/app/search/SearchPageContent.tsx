...
import { useState, useEffect, Suspense, memo, useCallback, useRef, useMemo } from 'react';
...
export default function SearchPageContent({ initial }: { initial?: { properties: PropertyCardType[]; nextToken: string | null } }) {
  ...
  const priceSort = filters.priceSort ?? (urlSort === 'price-low' ? 'asc' : urlSort === 'price-high' ? 'desc' : undefined);
  const sortBy = priceSort === 'asc' ? 'PRICE_LOW_HIGH' : 
                 priceSort === 'desc' ? 'PRICE_HIGH_LOW' : undefined;
  ...
  useEffect(() => {
    ...
    if (sortParam === 'price-low') initialFilters.priceSort = 'asc';
    if (sortParam === 'price-high') initialFilters.priceSort = 'desc';
  ...
  const handleFiltersChange = useCallback((newFilters: PropertyFilters) => {
    ...
    // Keep filters in the URL so returning from a listing (or sharing the link) restores the search.
    ...
    if (newFilters.priceSort) params.set('sort', newFilters.priceSort === 'asc' ? 'price-low' : 'price-high');
  ...
  // A rent of 0 means the listing is incomplete; keep it out of results.
  // Derived (not effect-synced) so the server-rendered first page includes the results.
  const filteredProperties = useMemo(() => properties.filter((p) => p.monthlyRent > 0), [properties]);
