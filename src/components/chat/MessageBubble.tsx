  ...
  return (
    <div className={`flex items-start space-x-2 sm:space-x-3 ${isOwnMessage ? 'flex-row-reverse space-x-reverse' : ''}`}>
      ...
      <div className="flex-shrink-0">
        <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm font-medium text-white ${
      ...
      <div className={`flex flex-col max-w-[75%] sm:max-w-[70%] ${isOwnMessage ? 'items-end' : 'items-start'}`}>
        ...
        {message.attachments && message.attachments.length > 0 && (
          <div className="mt-1.5 sm:mt-2 space-y-1.5 sm:space-y-2">
            {message.attachments.map((attachment) => (
              <div
                key={attachment.id}
                className={`flex items-center space-x-1.5 sm:space-x-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm ${
                  isOwnMessage
        ...
        <div className={`flex items-center space-x-1 mt-0.5 sm:mt-1 ${isOwnMessage ? 'flex-row-reverse space-x-reverse' : ''}`}>
