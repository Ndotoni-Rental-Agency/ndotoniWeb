...
function SocialCard({ platform, delay }: { platform: any; delay: number }) {
  ...
  return (
    <a
      ...
      rel="noopener noreferrer"
      ref={ref}
      className={`group block bg-white dark:bg-gray-800 rounded-2xl p-4 md:p-6 border-2 border-gray-200 dark:border-gray-700 hover:border-transparent transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
      }`}
    >
      <div className="space-y-3 md:space-y-4">
        {/* Icon with gradient background */}
        <div className={`w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br ${platform.color} ${platform.hoverColor} rounded-xl md:rounded-2xl flex items-center justify-center text-white shadow-md md:shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:rotate-3`}>
          {platform.icon}
        </div>
