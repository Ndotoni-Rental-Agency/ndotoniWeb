    ...
    <footer className="bg-cream-200 dark:bg-gray-900 border-t border-stone-200 dark:border-gray-700 transition-colors">
      ...
      <div className="border-b border-stone-200/70 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="grid grid-cols-1 md:grid-cols-[1.4fr,1fr] gap-8 items-end">
            <div>
              <p className="eyebrow mb-3">Ndotoni · Tanzania</p>
              <h2 className="font-display text-3xl sm:text-4xl md:text-5xl tracking-tight text-ink-900 dark:text-white text-balance">
                Pata makazi yanayoendana na maisha yako.
              </h2>
            </div>
            <p className="text-ink-500 dark:text-gray-400 text-base sm:text-lg max-w-md">
              {t('hero.subtitle')}
            </p>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-sm text-ink-500 dark:text-gray-400 mb-5 leading-relaxed">
              {t('hero.subtitle')}
            </p>
            ...
            <ul className="space-y-3">
              {footerLinks.company.map((link) => (
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="text-sm text-ink-500 dark:text-gray-400 hover:text-clay-700 dark:hover:text-white transition-colors"
                  >
                    {link.name}
                  </Link>
                </li>
        ...
        <div className="mt-12 pt-8 border-t border-stone-200 dark:border-gray-700">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex flex-col items-center md:items-start space-y-1">
              ...
              <p className="text-sm text-ink-700 dark:text-gray-300 font-medium">
                Ndotoni Online Traders
              </p>
              <p className="text-xs text-ink-500/80 dark:text-gray-500">
                Registration No: ***961 · Registered in Tanzania
              </p>
              <div className="hidden md:flex items-center space-x-5 mt-2">
                <Link href="/privacy" className="text-sm text-ink-500 dark:text-gray-400 hover:text-ink-900 dark:hover:text-white transition-colors">
                  {t('footer.privacy')}
                </Link>
                <Link href="/terms" className="text-sm text-ink-500 dark:text-gray-400 hover:text-ink-900 dark:hover:text-white transition-colors">
                  {t('footer.terms')}
                </Link>
                <Link href="/data-deletion" className="text-sm text-ink-500 dark:text-gray-400 hover:text-ink-900 dark:hover:text-white transition-colors">
                  Data Deletion
                </Link>
                <Link href="/sitemap" className="text-sm text-ink-500 dark:text-gray-400 hover:text-ink-900 dark:hover:text-white transition-colors">
                  {t('footer.sitemap')}
                </Link>
              </div>
            </div>
          ...
          <div className="md:hidden mt-5 flex flex-wrap items-center justify-center gap-x-4 gap-y-2">
            <Link href="/privacy" className="text-sm text-ink-500 dark:text-gray-400 hover:text-ink-900 dark:hover:text-white transition-colors">
              {t('footer.privacy')}
            </Link>
            <Link href="/terms" className="text-sm text-ink-500 dark:text-gray-400 hover:text-ink-900 dark:hover:text-white transition-colors">
