      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <PhoneInput
            ...
            helperText={t('profile.whatsappHelper')}
            disabled={!isEditing}
          />
        </div>
          ...
          {isEditing ? (
            ...
            <div>
              ...
              <div className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">
                {formData.dateOfBirth ? new Date(formData.dateOfBirth).toLocaleDateString() : t('profile.notSet')}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="md:col-span-2">
          ...
          {isEditing ? (
            <div>
              ...
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {t('profile.nationalIdOptional')}
              </p>
            </div>
          ) : (
            <div>
              <div className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">
                {formData.nationalIdLast4 ? `****${formData.nationalIdLast4}` : t('profile.notSet')}
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {t('profile.nationalIdSecurity')}
              </p>
            </div>
          )}
        </div>
      ...
      {isEditing && (
        <div className="mt-6 flex gap-2">
          <Button 
            ...
            disabled={isUpdating}
          >
            {isUpdating ? t('profile.saving') : t('profile.savePersonalInfo')}
          </Button>
          <Button 
            variant="outline" 
            size="sm"
