...
type ConfirmationVariant = 'brand' | 'destructive';
...
const variantStyles: Record<ConfirmationVariant, { iconWrap: string; icon: string; button: string }> = {
  brand: {
    iconWrap: 'bg-brand-50 dark:bg-brand-900/20',
    ...
    <div className="fixed inset-0 z-[1100] overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full p-6 transform transition-all">
          {/* Icon */}
          <div className={cn('mx-auto flex items-center justify-center h-12 w-12 rounded-full mb-4', styles.iconWrap)}>
            <svg className={cn('h-6 w-6', styles.icon)} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              {variant === 'destructive' ? (
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className={cn(
                ...
                confirmButtonClass ?? styles.button
              )}
            >
              {isLoading ? (
